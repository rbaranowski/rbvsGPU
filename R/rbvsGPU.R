#' @title Ranking-Based Variable Selection on GPU
#' @description The package provides routines extending the R package rbvs in order to perform some of the computations on a NVidia GPU.
#' @details The main routine of the package is \code{\link{rbvsGPU}}.
#' @docType package
#' @references R. Baranowski, P. Fryzlewicz (2016), Ranking-Based Variable Selection, in submission (\url{http://personal.lse.ac.uk/baranows/rbvs/rbvs.pdf)}).
#' @name rbvsGPU-package


NULL

#' @title Ranking-Based Variable Selection on GPU
#' @description Performs Rankings-Based Variable Selection using various measures of the dependence between the predictors and the response, performing computations on a GPU.
#' @details Currently supported measures are: Pearson correlation coefficient (\code{measure="pc"})
...
rbvsGPU <- function(x,y,...) UseMethod("rbvsGPU")

#' @rdname rbvsGPU
#' @method rbvsGPU default 
#' @useDynLib rbvsGPU matrix_projection_in_place_gpu_r
#' @export rbvsGPU.default
#' @importFrom rbvs s.est.quotient subsample rankings
#' @importFrom stats qr.Q


rbvsGPU.default <- function(x,
                            y,
                            m,
                            B=500,
                            measure=c("pc"),
                            s.est=s.est.quotient, 
                            iterative=TRUE, 
                            use.residuals=TRUE, 
                            k.max,min.max.freq=0,
                            max.iter=10, 
                            verbose = TRUE, ...){
  ...
  while(i <= min(no.iterations,max.iter)){
    ...
    omega <- fun(x.current, y.current, subsample(n,m,B),  ...)
    ...
    if(iterative & tmp$s.hat>0) {
      ...
      rm(x.current)
      x.current <- .Call("matrix_projection_in_place_gpu_r", x, PI, rbvs.object$active)
